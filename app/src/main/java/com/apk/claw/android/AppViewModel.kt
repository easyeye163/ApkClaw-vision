    ...
    fun startChatTask(task: String, imageData: ByteArray?, callback: ChatActivity.ChatCallback) {
        ...
        if (imageData != null) {
            try {
                ...
                XLog.e(TAG, "Failed to save chat image", e)
            }
        }

        val taskWithContext = if (imageData != null) {
            "$task\n\n(Note: User has attached an image. Use get_screen_info to understand context if needed.)"
        } else {
            task
        }

        taskOrchestrator.startNewTask(fakeChannel, taskWithContext, fakeMessageId)

        android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
            if (!isTaskRunning()) {
                callback.onComplete("Task started")
            }
        }, 100)
    }
