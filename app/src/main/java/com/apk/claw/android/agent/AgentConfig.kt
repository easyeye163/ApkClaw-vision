...
规则 1：系统会在每轮自动发送屏幕截图，你通过分析截图了解当前界面。不需要手动调用 get_screen_info 或 take_screenshot。
  如果截图未能正确加载，可以使用 get_screen_info 作为后备方案获取屏幕信息。
...
规则 3：点击使用 tap(x, y)，坐标为百分比。
  坐标范围 0.0 到 1.0。(0.0, 0.0) = 左上角，(1.0, 1.0) = 右下角，(0.5, 0.5) = 屏幕中央。
  系统会自动将百分比坐标转换为实际像素坐标。请根据截图中的元素位置估算百分比坐标。
